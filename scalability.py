...
def _make_cache_name(method: str, csv_file: str, X_shape: tuple, params: dict) -> str:
    mtime = str(os.path.getmtime(csv_file)) if csv_file and os.path.exists(csv_file) else "nofile"
    key = json.dumps({"method": method, "mtime": mtime, "shape": X_shape, "params": params}, sort_keys=True)
    h = hashlib.sha1(key.encode('utf-8')).hexdigest()[:16]
    return f"{method}_{h}.npz"

def _load_or_save_embedding(cache_path: str, compute_fn):
    os.makedirs(os.path.dirname(cache_path), exist_ok=True)
    if os.path.exists(cache_path):
        try:
            data = np.load(cache_path, allow_pickle=True)
            if 'X_2d' in data:
                print(f"Loaded embedding from cache: {cache_path}")
                return data['X_2d']
        except Exception:
            pass
    X_2d = compute_fn()
    np.savez_compressed(cache_path, X_2d=X_2d)
    print(f"Saved embedding to cache: {cache_path}")
    return X_2d

def compute_tsne(X_scaled, csv_file=None, cache_dir='cache', n_components=2, perplexity=30, max_iter=3000, random_state=42):
    params = {"n_components": n_components, "perplexity": perplexity, "max_iter": max_iter, "random_state": random_state}
    cache_name = _make_cache_name("tsne", csv_file, X_scaled.shape, params)
    ...
    return _load_or_save_embedding(cache_path, _compute)

def compute_isomap(X_scaled, csv_file=None, cache_dir='cache', n_components=2, n_neighbors=5):
    params = {"n_components": n_components, "n_neighbors": n_neighbors}
    cache_name = _make_cache_name("isomap", csv_file, X_scaled.shape, params)
    cache_path = os.path.join(cache_dir, cache_name)
...
def visualize_neighbourhoods_2d(features_df, query_index, neighbor_indices, X_2d, show=True):
    ...
    plt.title('2D Visualization of Item Neighbourhoods')
...
if __name__ == '__main__':
    ...
    nn_model.fit(X_2d)
    print("k-NN model built on Isomap map.")
