...
def _find_comparison_algo_file(root_dir):
    ...
    for subdir, _, files in os.walk(root_dir):
        for f in files:
            ...
            if "comparison" in name and "algo" in name and name.endswith(".py"):
                return os.path.join(subdir, f)
    raise FileNotFoundError("Could not locate the comparison algorithm file (e.g., 'Comparison algo.py').")
...
def _category_from_label(label: str) -> str:
    ...
    # Paths are normalized to '/' in the comparison algo, so we do the same here.
    return label.replace("\\", "/").split("/")[0]

def _default_group_weights() -> dict:
    """Provides default weights for feature groups."""
    # These weights can be adjusted to match the ones in the main script if needed.
    return {
        'A3': 3.0,
        'D1': 1.0,
        'D2': 2.0,
...
def init_worker(algo_path, feature_csv_path, obj_root_dir, group_weights):
    ...
    # The comparison algorithm now expects a CSV path
    engine_instance = algo_mod.ShapeSearchEngine(feature_csv_path, obj_root_dir, group_weights)
...
def evaluate_chunk(query_labels_chunk: list, metric: str, top_n: int) -> tuple[int, int]:
    ...
    for query_label in query_labels_chunk:
        try:
            results = engine_instance.search(query_label, top_n=top_n, metric=metric)
...
def main():
    ...
    feature_csv = os.path.join(dataset_root, "all_features.csv")
    ...
    # Import the comparison algorithm module to use load_dataset
    algo_mod = _import_module_from_path("comparison_algo", algo_path)

    # Load all labels using the updated load_dataset function
    # Assuming the CSV has headers (set has_header=True; adjust if no headers)
    df, X, categories, obj_ids = algo_mod.load_dataset(feature_csv, models_dir=None, has_header=False)
    all_labels = [os.path.join(cat, obj) for cat, obj in zip(categories, obj_ids)]

    if not all_labels:
        raise ValueError("No labels found in the CSV file. Check the 'object' and 'category' columns.")

    group_weights = _default_group_weights()
    metrics_to_test = sorted(['euclidean', 'manhattan', 'emd'])#'cosine', 'emd', 'chi-squared', 'kullback-leibler', 'cross-bin'])
    num_workers = max(1, cpu_count() - 1)

    print(f"Evaluating {len(metrics_to_test)} metrics with top_n={top_n} on {len(all_labels)} shapes using {num_workers} workers...")
    print("-" * 50)

    # --- 2. Evaluate All Metrics ---
    results = {}
    init_args = (algo_path, feature_csv, obj_root_dir, group_weights)

    for metric in metrics_to_test:
        metric_start_time = time.time()
        print(f"Evaluating metric: {metric}...")

        chunk_size = max(1, len(all_labels) // (num_workers * 4))
    ...
    sorted_results = sorted(results.items(), key=lambda item: item[1][2], reverse=True)
    for metric_name, (correct, total, percent) in sorted_results:
